...
import pathlib
from tomlkit import parse, inline_table, dumps
...
class PackageGenerator:
        ...
        for template_name in available_templates:
            ...
            if template_config:
                ...
                choices.append(questionary.Choice(
                    title=f"{template_name}: {description}",
                    value=template_name
                ))
    ...
    def register_package_in_workspace(self, project_root: pathlib.Path, package_name: str) -> None:
        ...
        known_first_party = data.get("tool", {}).get("ruff", {}).get("lint", {}).get("isort", {}).get("known-first-party", [])
        ...
        if package_pattern not in members:
            ...
            with open(pyproject_path, "w", encoding="utf-8") as f:
                f.write(dumps(data))
            logger.info(f"Registered package '{package_name}' in workspace and sources, and saved updated pyproject.toml")
        else:
