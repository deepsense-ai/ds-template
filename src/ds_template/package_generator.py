...
class PackageGenerator:
    ...
    def find_project_root(self, start_path: pathlib.Path | None = None) -> pathlib.Path:
        """
                ...
                try:
                    ...
                    if isinstance(tool_section, dict):
                        ...
                        if isinstance(uv_section, dict):
                            workspace_config = uv_section.get("workspace")
                            if isinstance(workspace_config, dict) and "members" in workspace_config:
    ...
    def generate_package(
        self, template_name: str, package_name: str, output_dir: pathlib.Path | None = None, **kwargs: dict[str, Any]
    ) -> pathlib.Path:
        """
    ...
    def register_package_in_workspace(self, project_root: pathlib.Path, package_name: str) -> None:
        ...
        # Workspace members handling (custom tooling)
        tool = data.setdefault("tool", {})
        uv = tool.setdefault("uv", {})
        workspace = uv.setdefault("workspace", {})
        members = workspace.setdefault("members", [])

        package_pattern = f"packages/{package_name}"

        # Ruff known-first-party handling
        ruff = tool.setdefault("ruff", {})
        lint = ruff.setdefault("lint", {})
        isort = lint.setdefault("isort", {})
        known_first_party = isort.setdefault("known-first-party", [])
        ...
        # Register package as a path dependency if not already present
        if package_name not in dependencies:
            dependencies[package_name] = {"path": f"packages/{package_name}"}

        # Sources section for uv
        sources = uv.setdefault("sources", {})
        if package_name not in sources:
            table = inline_table()
            table["workspace"] = True
            sources[package_name] = table

        # Write updated pyproject.toml
        with open(pyproject_path, "w", encoding="utf-8") as f:
            f.write(dumps(data))

        logger.info(f"Registered package '{package_name}' in workspace and poetry dependencies, updated pyproject.toml")
